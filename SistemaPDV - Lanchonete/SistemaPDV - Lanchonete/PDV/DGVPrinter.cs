...
{    
    ...
    public class DGVPrinter
    {
        ...
        /// 
            ...
            public string cliente; //Atributo da classe
            public string endereco; //Atributo da classe
            private string numero; //Atributo da classe
            private string bairro; //Atributo da classe
            private string formaPagamento; //Atributo da classe
            private string formaEntrega; //Atributo da classe
            private string Troco; //Atributo da classe
            private string valorTroco; //Atributo da classe
            private string taxaEntrega; //Atributo da classe
            private string subTotal; //Atributo da classe
            private string total; //Atributo da classe
            private string nPedido; //Atributo da classe
        ...
        private void Pd_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (printDocument != null)
            {
                ...
                {
                    e.Graphics.DrawString($"NUMERO PEDIDO: {nPedido}", font, brush, 150, margem);
                    e.Graphics.DrawString($"CLIENTE: {cliente}", font, brush, 0, margem=margem+20);
                    e.Graphics.DrawString($"ENDEREÇO: {endereco}", font, brush, 0, margem=margem+20);
                    e.Graphics.DrawString($"NUMERO: {numero}", font, brush, 0, margem = margem + 20);
                    e.Graphics.DrawString($"BAIRRO: {bairro}", font, brush, 0, margem = margem + 20);
        ...
        private void Pd_PrintPage2(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (printDocument != null)
            {
                ...
                using (var brush = new SolidBrush(Color.Black))
                {
                    e.Graphics.DrawString($"SUB TOTAL: {subTotal}", font, brush, 0, margem=50);
                    e.Graphics.DrawString($"TAXA {formaEntrega}: {taxaEntrega}", font, brush, 0, margem = margem + 20);
                    e.Graphics.DrawString($"TOTAL: {total}", font, brush, 0, margem = margem + 20);
                    e.Graphics.DrawString($"FORMA PGTO: {formaPagamento}", font, brush, 0, margem = margem + 20);
                    e.Graphics.DrawString($"TROCO PARA: {Troco}", font, brush, 0, margem = margem + 20);
                    e.Graphics.DrawString($"TROCO DO CLIENTE: {valorTroco}", font, brush, 0, margem = margem + 20);
        ...
        public void setDadosVendas(
            string nome, string end, string n, 
            string bair, string fPgto, string fEntrega, 
            string troc, string VTroco, string tax, 
            string sTotal, string tot, string nPed)
        {
            ...
            numero = n;
            bairro = bair;
            formaPagamento = fPgto;
            formaEntrega = fEntrega;
            Troco = troc;
            taxaEntrega = tax;
            subTotal = sTotal;
            total = tot;
        ...
        public void PrintDataGridView(DataGridView dgv)
        {
            ...
            if (DialogResult.OK == DisplayPrintDialog())
            {
                ...
                using (var printDocument = new System.Drawing.Printing.PrintDocument())
                {
                    printDocument.PrintPage += Pd_PrintPage2;
                    printDocument.Print();
                }
            }
        }
